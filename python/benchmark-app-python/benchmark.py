...
def main(args=None):
    try:
        ...
        logger.info("Device: {}".format(plugin.device))
        logger.info("Plugin version: {}".format(plugin.version))
        ...
        input_images = {key: np.ndarray(input_info[key].shape)}
        ...
        if args.api_type == 'sync':
            ...
            if args.number_iterations is not None:
                ...
                logger.info("Start inference synchronously ({}sec duration)".format(duration))
            ...
            print("[BENCHMARK RESULT] Latency: {:.4f} msec".format(latency * 1e3))
            print("[BENCHMARK RESULT] Throughput: {:.4f} FPS".format(fps))

            with open(stats_file, 'w') as f:
                f.write("{:.2f}".format(fps) + '\n')
            ...
            if args.number_iterations is not None:
                ...
                logger.info("Start inference asynchronously ({}sec duration, "
            ...
            while not required_inference_requests_were_executed or step < steps_count or \
                ...
                step += 1
                if step % 10 == 0:
                    simpleProgressUpdate(progress_file_path, (datetime.now() - start_time).total_seconds(), duration) 
